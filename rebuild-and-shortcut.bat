...
echo [ThaiAsia] Cai electron-packager neu chua co...
call npm install --save-dev electron-packager

echo [ThaiAsia] Build lai app voi icon...
call npx electron-packager . ThaiAsiaApp --platform=win32 --arch=x64 --icon=icon.ico --out=dist --overwrite --ignore=dist --ignore=node_modules\.cache

echo [ThaiAsia] Tao shortcut tren Desktop...
set "EXE=%APP_DIR%dist\ThaiAsiaApp-win32-x64\ThaiAsiaApp.exe"
...
echo [ThaiAsia] Xong! Da tao 2 icon tren Desktop:
echo   - ThaiAsia AllInOne     (tu dong watchdog)
